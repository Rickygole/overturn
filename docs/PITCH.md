...
- **Retrieval** finds the governing section of the insurer's own policy corpus by TF-IDF over unigrams and order-independent word pairs, falling back to a model-assisted query reformulation only when the first pass scores below a measured similarity floor. The thresholds are set from a calibration run over every case rather than by intuition, and the calibration script fails rather than suggesting a threshold when the correct-policy and no-policy score populations overlap. Retrieval returns *whole policies* rather than top-k sections, because a criteria list truncated by rank is a criteria list with silent holes in it.
...
Seven agents, one job each, each under its own service account, traced end to end in Cloud Trace. Model Armor screens every inbound document. Where a managed platform component had no reachable surface on my project, I built the same contract on primitives and wrote down which is which rather than implying otherwise.
