...
| **Memory Bank** | Primitive — `core/memory.py` over the `case_memory` collection | No public REST surface on this project. The same contract is implemented on Firestore: cross-case observations keyed on payer, policy and denial reason code, and never on a member. That scoping is a schema decision rather than a policy anyone has to follow. |
